...
library(plotly)
library(htmlwidgets)
>>>>>>> 2d3d4fef64b0adaa9559cb48d792df409afda279
```
...
=======
colores <- c("Canada" = "orange", "France" = "green", "Germany" = "yellow",
             "Italy" = "cyan", "Japan" = "pink", "Spain" = "red",
             "United Kingdom" = "purple", "United States" = "grey")

grafico <- ggplot(data = covid_data_ref, aes(x = date, 
                              y = new_deaths_per_million/7, color = location)) +
  geom_line(aes(linewidth = location)) + scale_color_manual(values = colores) +
  scale_discrete_manual("linewidth", values = c(0.5, 0.5, 0.5, 0.5, 0.5, 1, 0.5
                                                , 0.5)) + 
  theme_bw() + theme(panel.grid = element_blank()) + 
  geom_hline(yintercept = c(0,5,10,15,20),linetype = "dashed", color = "grey") + 
  theme(plot.caption = element_text(hjust = 0))

grafico_interactivo <- ggplotly(grafico)

grafico_interactivo <- grafico_interactivo %>% 
  layout(title = list(text = paste0("Grupo F: Daily new confirmed COVID-19 deaths per million people", "<br>", "<sup>", "For some countries the number of confirmed deaths is much lower than the true number of deaths."), x = 0), margin = list(t = 50))

print(grafico_interactivo)

saveWidget(grafico_interactivo, file ="GraficoTema3.html", selfcontained = TRUE)#Guardar figura
>>>>>>> 2d3d4fef64b0adaa9559cb48d792df409afda279
```
